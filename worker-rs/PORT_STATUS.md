...
| `inbox-fallback.ts` | `src/inbox_fallback.rs` + `routes_channels.rs` | **ported, then deliberately diverged** | No longer a fallback: the Worker is the primary responder and claims an inbox item as the webhook lands, under `waitUntil` (`answer_and_deliver_now`), with the cron sweep left as the retry net. The TS 2-min claim threshold is now `CHANNEL_RESPONDER_CLAIM_DELAY_MS`, default 0. The desktop-flavoured copy is gone with it: the non-Pro reply names the plan that is missing (`NO_PLAN_TEXT`), the exhausted-attempts reply names the failure (`MODEL_UNAVAILABLE_TEXT`), and the system prompt no longer tells the model it is standing in for an offline desktop. Lease claim/fencing (`channel_inbox` UPDATE…RETURNING + `lease_token` guard), retry/failed release transitions, non-Pro static ack, final-attempt ack, `CHANNEL_FALLBACK_RESPONDER` kill switch, prompt assembly + reply trim/cap. Cross-group calls now WIRED: `runManagedInboxCompletion` → `routes_ai::run_managed_inbox_completion` (admission DO admit/settle/release + `managed_ai_requests` ledger + non-streaming MIMO completion), `memoryContextFor` → `routes_memory::memory_context_for` (single Vectorize impl), `completeInboxItemDone` → `glue::complete_inbox_done` (delivery + conversation-message batch with the `Channel is not linked` re-read). 10 pure unit tests. |
