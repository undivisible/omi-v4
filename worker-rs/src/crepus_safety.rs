...
/// `onchange`, `onlongpress`, `on-long-press`, `on_long_press` and the
/// hyphen/underscore spellings in between.
...
/// Scans forward from `start` (the byte index just past a matched attribute
/// name) for `[_-]?=` then the value, returning the decoded value and the byte
...
pub fn find_disallowed_crepus_action(source: &str) -> Option<String> {
    ...
    while let Some(offset) = lower[index..].find("on") {
        ...
        let Some((action, resume)) = read_attribute_value(source, after) else {
            continue;
        };
        index = resume;
...
pub fn find_unsafe_crepus_image_url(source: &str) -> Option<String> {
    ...
    while line_start <= lower.len() {
        let line_end = lower[line_start..]
            ...
            .unwrap_or(lower.len());
        if let Some(url) = unsafe_image_url_in_line(source, &lower, line_start, line_end) {
            return Some(url);
        }
...
fn unsafe_image_url_in_line(
    ...
    while let Some(offset) = line[search..].find("src=") {
        ...
        let raw_start = line_start + at + 3;
        let raw = read_image_src(source, raw_start, line_end)?;
...
mod tests {
    ...
    #[test]
    fn reads_the_quoted_and_bare_action_spellings() {
        for source in [
