...
pub fn sanitize_channel_reply(channel: &str, text: &str) -> String {
    ...
    for (marker, keep_when_empty) in [("**", false), ("__", false), ("*", false), ("_", false)] {
        let _ = keep_when_empty;
        value = unwrap_emphasis(&value, marker);
    }
    value = unwrap_emphasis(&value, "`");
...
fn unwrap_markdown_links(value: &str) -> String {
    ...
    while index < bytes.len() {
        if bytes[index] == b'[' {
            if let Some(label_end) = value[index + 1..].find(']') {
                let label_end = index + 1 + label_end;
                if label_end + 1 < bytes.len() && bytes[label_end + 1] == b'(' {
                    if let Some(url_end) = value[label_end + 2..].find(')') {
...
fn unwrap_emphasis(value: &str, marker: &str) -> String {
    let inner_char = marker.as_bytes()[0] as char;
    let mut out = String::with_capacity(value.len());
    let mut rest = value;
    while let Some(open) = rest.find(marker) {
        let after = &rest[open + marker.len()..];
        let body_end = match after.find(inner_char) {
