...
static void wifi_ap_stations_unlocked(void)
{
	...
	for (int i = 0; i < AP_MAX_STATIONS; i++) {
		...
		LOG_INF("Link mode: %u", (unsigned)sta->link_mode);
...
static void cmd_wifi_status(void)
{
	...
	LOG_INF("==================");
	LOG_INF("State: %u", (unsigned)status.state);

	if (status.state >= WIFI_STATE_ASSOCIATED) {
		uint8_t mac_string_buf[sizeof("xx:xx:xx:xx:xx:xx")];

		LOG_INF("Interface Mode: %u", (unsigned)status.iface_mode);
		LOG_INF("Link Mode: %u", (unsigned)status.link_mode);
		LOG_INF("SSID: %.32s", status.ssid);
		LOG_INF("BSSID: %s",
			net_sprint_ll_addr_buf(status.bssid,
					       WIFI_MAC_ADDR_LEN, mac_string_buf,
					       sizeof(mac_string_buf)));
		LOG_INF("Band: %u", (unsigned)status.band);
		LOG_INF("Channel: %d", status.channel);
		LOG_INF("Security: %u", (unsigned)status.security);
		LOG_INF("MFP: %u", (unsigned)status.mfp);
...
void start_wifi_thread(void)
{
	...
	while (1) {
		...
		case OMI_WIFI_STATE_CONNECTING:
