...
#define DISK_DRIVE_NAME DT_PROP(DT_CHILD(DT_NODELABEL(sdhc0), mmc), disk_name)
...
static int advance_read_seq_internal(uint64_t new_read_seq)
{
    ...
    handle_req:
        ...
        case REQ_ADVANCE_READ:
            if (req.u.advance.resp) {
                req.u.advance.resp->res = advance_read_seq_internal(req.u.advance.new_read_seq);
