...
describe("oauth subscription chat proxy", () => {
  const chat = (
    ...
    request(
      ...
      { OPENAI_OAUTH_CLIENT_ID: "app_test", XAI_OAUTH_CLIENT_ID: "xai_test" },
    );
  ...
  test("refreshes an expired token, persists rotation, and streams through", async () => {
    ...
    await database
      .prepare(
        "INSERT INTO oauth_connections (uid, provider, access_token, refresh_token, account_id, expires_at, created_at, updated_at) VALUES ('pro', 'openai', 'stale-access', 'old-refresh', 'acct-1', ?1, ?2, ?2)",
      )
      .bind(now - 1000, now)
      .run();
  ...
  test("401 reconnect required when the refresh is rejected", async () => {
    ...
    await database
      .prepare(
        "INSERT INTO oauth_connections (uid, provider, access_token, refresh_token, expires_at, created_at, updated_at) VALUES ('byok', 'openai', 'stale-access', 'bad-refresh', ?1, ?2, ?2)",
      )
      .bind(now - 1000, now)
      .run();
    globalThis.fetch = (async () =>
  ...
  test("passes upstream status through for a live token", async () => {
    ...
    await database
      .prepare(
        "INSERT INTO oauth_connections (uid, provider, access_token, refresh_token, expires_at, created_at, updated_at) VALUES ('pro', 'xai', 'grok-access', 'grok-refresh', ?1, ?2, ?2)",
      )
      .bind(now + 3_600_000, now)
      .run();
    let upstreamUrl = "";
    let upstreamAuth: string | null = null;
    globalThis.fetch = (async (
