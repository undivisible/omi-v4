...
- **The Rust worker is production.** `worker-rs` serves `omi.tsc.hk` and `api.omi.tsc.hk`; the TS worker remains for test parity and rollback only. FaceTime is not ported (501 until bridge container lands).
