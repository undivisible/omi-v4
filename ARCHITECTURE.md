...
flowchart TD
    ...
    subgraph Worker["Bun/Hono Cloudflare Worker — worker/src"]
        ...
        STTW["stt.ts / voice.ts / asr.ts<br/>legacy STT sessions, Gemini Live tokens,<br/>OpenRouter Grok STT batch transcription"]
    ...
    subgraph External["External providers"]
        FB["Firebase Auth"]
        DG["Gemini Live (realtime voice)<br/>OpenRouter Grok STT (batch)<br/>Deepgram (legacy STT)"]
...
**Opt-in third-party reporting:** the Rust Worker forwards Tail batches to Better Stack Logs, pings a Better Stack heartbeat after a successful cron batch, sends Sentry-compatible error envelopes to Better Stack Errors, and sends content-free managed-LLM traces to foglamp.dev. Every path is disabled unless its corresponding secret is configured. The Flutter client remains unwired.
...
- **Observability account setup remains external.** Workers Observability and the Rust worker's opt-in Better Stack and foglamp transports are wired. Creating uptime monitors, a status page, an on-call policy, log/error sources, and API credentials requires the corresponding vendor accounts; the Flutter client remains unwired.
...
- **The Rust worker is production.** `worker-rs` serves `omi.tsc.hk` and `api.omi.tsc.hk`; the TS worker remains for test parity and rollback only. FaceTime is ported: the Sendblue call flow, the MCP tool, and the `FACETIME_BRIDGE` container binding are all wired, and the call is refused with `facetime_unavailable` until `SENDBLUE_FACETIME_NUMBER` is configured.
