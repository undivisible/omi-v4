...
- [x] **Embeddings:** use local FastEmbed BGE queries against Cloudflare Workers AI (`bge-base`) claim vectors; do not create image embeddings
...
- [ ] **Speech profiles:** the hub foundation is in (`app/native/hub/src/speech_profiles.rs`: SQLite store, threshold+margin matching, merge/rename/forget/pause/quarantine, provenance, Rinf signals — see `docs/speech-profiles-and-displays.md` §8.1). The remaining blocker is **no local speaker-embedding model ships yet**, not a vendor gap: no provider will ever supply this. `x-ai/grok-stt-1.0` returns text only, Deepgram's `diarize=true` returns request-scoped indices, Gemini Live returns its own transcription (`docs/why-grok-stt-cannot-do-voiceprints.md`). Next step is picking an on-device model to put behind `EmbeddingSource`, then re-measuring `MATCH_THRESHOLD` / `MATCH_MARGIN` against it.
