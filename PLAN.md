...
| Cloud | Production: Bun/TypeScript Hono Worker (`worker/`) owns D1 migrations and `omi.tsc.hk`. Destination: Rust `worker-rs/` shadow → cutover per `worker-rs/CUTOVER.md`. D1, R2, Queues, Workflows, Durable Objects where coordination requires them |
