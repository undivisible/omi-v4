...
These match upstream's constants exactly (`~/projects/omi/app/lib/services/devices/models.dart:12-38`). Everything else upstream defines — button, image capture, SD-card storage, accelerometer, time sync, Device Information (`180a`), settings service (`19b10010`) — is not referenced anywhere in `app/lib`.
...
- **Accelerometer stream** (`32403790-…`), **time sync** (`19b10030-…`), **Device Information service** (`180a`: model/firmware/hardware/manufacturer/serial), and a **settings service** (`19b10010`: dim ratio, mic gain, charging status).
...
| Device Information service (`180a`) reads | `services/devices/models.dart:40-45` | **Worth adopting; low cost.** `RelayDevice` already has the fields and the UI already has the tile (§10). Phase 2. |
...
- **The Firmware tile always reads "Not reported".** `RelayDevice` has `modelNumber`, `firmwareRevision`, `hardwareRevision`, `manufacturerName`, and `serialNumber` fields, but `_relayDevice()` never populates them — the Device Information service (`180a`) is not read. The UI tile at `mobile_companion_shell.dart:458-463` therefore renders a permanent placeholder.
...
- **The capture toggle is asymmetric.** Turning capture off calls `deviceAudio.stop()`; turning it back on calls `connectDevice(device.id)` again — i.e. a full reconnect-and-restart rather than resuming a streaming session (`mobile_companion_shell.dart:384-397`).
- **Diagnostic `print` calls ship in the BLE adapter.** Three `// ignore: avoid_print` sites in `universal_ble_device_relay.dart` log connect failures to stdout in release builds.
